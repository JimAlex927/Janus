...
- An optional loopback-only admin listener exposes `/livez`, `/readyz` and
  ...
  request IDs and bounded request outcome records; the gateway-generated ID is
  authoritative on the final response even if a backend supplies its own value.
  Server and backend deadlines are validated
...
- HTTP/3 uses a separately bound UDP socket, advertises `Alt-Svc` from the TCP
  ...
  forwarding, local UDP behavior, serve-failure cleanup and TCP fallback stop
  on an injected UDP read error are tested; Linux
